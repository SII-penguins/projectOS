...
Use unambiguous headings. Shared metadata belongs before the first heading at the selected section's level. Other section bodies are excluded, so history rows do not become live tasks. Nested subsections belong to their selected parent.
...
- A run table contains a trust field; artifact content and scientific validity are not checked.
